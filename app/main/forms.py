...
class JobDateForm(FlaskForm):
    ...
    run_date = StringField(
        "计划运行时间", default="2019-03-20 11:11:11", validators=[Required()]
    )
...
class JobIntervalForm(FlaskForm):
    ...
    func_cmd = StringField("执行函数或命令", validators=[Required()])
    interval_time = StringField(
        "计划运行时间(秒 分 时 日 周)", default="10 0 0 0 0", validators=[Required()]
    )
